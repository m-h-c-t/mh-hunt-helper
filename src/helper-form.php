...
<form novalidate name="item_list_form" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
    <div class="form-group row">
        ...
        <textarea rows="10" class="form-control input-lg" placeholder="Copy map <?php echo $name; ?> list here..." required name="<?php echo $name; ?>" id="<?php echo $name; ?>"><?php if (isset($results['original_items']) && !empty($results['original_items'])) { echo implode("\n", $results['original_items']); } ?></textarea>
    </div>
    <div class="form-group row">
