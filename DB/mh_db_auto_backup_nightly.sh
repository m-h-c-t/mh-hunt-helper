...
if [ -f hunthelper_nightly.sql.gz ]; then
	rm hunthelper_nightly_txt.zip
fi

if [ -f hunthelper_nightly_txt.zip ]; then
	rm hunthelper_nightly_txt.zip
fi

mysqldump -u $MH_USER -p$MH_PASS --host=127.0.0.1 --skip-lock-tables --events mhhunthelper | gzip -9 > hunthelper_nightly.sql.gz
sleep 5s
...
zip -j -9 hunthelper_nightly_txt.zip /var/lib/mysql-files/*
rm -rf /var/lib/mysql-files/*
